...
TOWER_HEIGHT = int(SCR_HEIGHT // (700 / 350)) # 700: original screen height, 350: original rect height

DISK_SIZE = TOWER_HEIGHT // (DISK_TOTAL + 3)
TOWER_WIDTH = DISK_SIZE // 2

DISK_RADIUS = int(DISK_SIZE // (43 / 7)) # 43: original disk size, 7: original corner radius
BORDER_WIDTH = DISK_SIZE // 10 + 3

RESET_WIDTH = int(SCR_WIDTH // (800 / 110)) # 800: original screen width, 110: original rect width
RESET_HEIGHT = int(SCR_HEIGHT // (700 / 60)) # 700: original screen height, 60: original rect height
...
TOWER_POSITIONS = list(py.Rect(x, TOWER_BASE, TOWER_WIDTH, TOWER_HEIGHT) for x in [TOWER1_X, TOWER_CENTER, TOWER3_X])
TRIGGER_POSITIONS = list(rect.copy().inflate(MOUSE_OFFSET * 2, 0) for rect in TOWER_POSITIONS)
...
def drawDisks(stack: list, tower_position: py.Rect):
